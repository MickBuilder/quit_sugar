...
class AppLogger {
  ...
  static void logApiError(String message, [dynamic error, StackTrace? stackTrace]) {
    _logger.e('❌ API Error: $message', error: error, stackTrace: stackTrace);
  }

  // Sugar tracking logging
  static void logSugarTracking(String message, [dynamic error, StackTrace? stackTrace]) {
    _logger.i('🍯 Sugar Tracking: $message', error: error, stackTrace: stackTrace);
  }

  static void logSugarTrackingError(String message, [dynamic error, StackTrace? stackTrace]) {
    _logger.e('❌ Sugar Tracking Error: $message', error: error, stackTrace: stackTrace);
  }

  // Scanner logging
  static void logScanner(String message, [dynamic error, StackTrace? stackTrace]) {
    _logger.i('📱 Scanner: $message', error: error, stackTrace: stackTrace);
  }

  static void logScannerError(String message, [dynamic error, StackTrace? stackTrace]) {
    _logger.e('❌ Scanner Error: $message', error: error, stackTrace: stackTrace);
  }

  // Navigation logging
  static void logNavigation(String message, [dynamic error, StackTrace? stackTrace]) {
  ...
  static void logState(String message, [dynamic error, StackTrace? stackTrace]) {
  ...
  static void logSugarCalculation(String productName, double sugarPer100g, double portion, double calculatedSugar) {
    _logger.i('🧮 Sugar Calculation: $productName - ${sugarPer100g}g/100g × ${portion}g = ${calculatedSugar}g');
  }
